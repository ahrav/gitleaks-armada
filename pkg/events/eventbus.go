...
type PublishOption func(*PublishOpt)

// PublishOpt contains configuration settings for event publishing.
...
func WithKey(key string) PublishOption {
	return func(opts *PublishOpt) { opts.Key = key }
}
...
func WithHeaders(headers map[string]string) PublishOption {
	return func(opts *PublishOpt) { opts.Headers = headers }
}
...
type EventBus interface {
	...
	Subscribe(ctx context.Context, eventTypes []domain.EventType, handler func(context.Context, DomainEvent) error) error
