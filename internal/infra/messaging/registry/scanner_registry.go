...
type ScannerRegistry struct {
	mu       sync.RWMutex
	scanners map[string]*connections.ScannerConnection
	metrics  GatewayMetrics
}

// NewScannerRegistry creates a new scanner registry.
func NewScannerRegistry(metrics GatewayMetrics) *ScannerRegistry {
	return &ScannerRegistry{scanners: make(map[string]*connections.ScannerConnection), metrics: metrics}
}

// Register adds a scanner connection to the registry.
// If a scanner with the same ID already exists, it will be replaced.
// This call always succeeds.
func (r *ScannerRegistry) Register(ctx context.Context, scannerID string, conn *connections.ScannerConnection) {
...
func (r *ScannerRegistry) Get(scannerID string) (*connections.ScannerConnection, bool) {
	r.mu.RLock()
	defer r.mu.RUnlock()
